...
const AdapterOutboundFile = {
		...
		if (message.header.file_name) {
			...
			if (this.directory) {
				...
				if (path.isAbsolute(fileName)) {
					...
					this.error({
						"message": message,
						"txt": "If there is no directory in the step configuration, then the file names must be absolute",
			...
			this.error({
				"message": message,
				"txt": "No 'file_name' property in the header",
				"endpoint": "inWriteFile"
			});
		}
	},
	...
	_writeFile(message, fileName) {
		...
		if (stream) {
			...
			this.error({
				"message": message,
				"txt": "The payload of the message has no stream",
				"endpoint": "inWriteFile"
			});
		}
	}
